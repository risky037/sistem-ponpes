        ...
        <tbody>
            @forelse ($result['data'] as $row)
                <tr>
                    ...
                    <td>{{ $row->academicYear?->name ?? '-' }} ({{ $row->academicYear?->semester ?? '-' }})</td>
                    <td>{{ $row->kelas ? $row->kelas->tingkatan . ' - ' . $row->kelas->kelas : '-' }}</td>
                    <td><strong>{{ $row->mapel?->name ?? '-' }}</strong></td>
                    <td class="text-center"><code>{{ $row->mapel?->code ?? '-' }}</code></td>
                    <td>{{ $row->user?->name ?? '-' }}</td>
                    <td class="text-center">{{ $row->status ?? '-' }}</td>
                    <td>
                        @if ($row->relationLoaded('classSchedules') && $row->classSchedules->isNotEmpty())
                            {{ $row->classSchedules->map(fn($s) => $s->day_of_week . ' (' . $s->start_time . '-' . $s->end_time . ')')->implode(', ') }}
                        @else
                            -
                        @endif
                    </td>
            ...
            @empty
                <tr>
                    <td colspan="8" class="text-center">Tidak ada data penugasan yang sesuai dengan filter.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
